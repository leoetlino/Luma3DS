...
// A small utility class that provides file or stream-like functionality
...
public:
  ...
  void Read(void* buffer, u32 read_length) {
    if (m_offset + read_length > m_size) {
  ...
  void Read(void* buffer, u32 read_length, u32 offset) {
    Seek(offset);
    Read(buffer, read_length);
  }
